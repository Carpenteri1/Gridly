...
public class ProviderKeysHandlerTests
{
    ...
    private static RemoteProvidersHandler CreateHandler(
        FakeLocalProvidersRepository repository,
    ...
    public async Task HandleGetStatus_WhenNoKeyStored_ReturnsExistsFalseWithoutCallingProvider()
    {
        ...
        var handler = CreateHandler(new FakeLocalProvidersRepository(), endPoint);

        var result = await handler.Handle(new GetRemoteProviderKeyStatusQuery { Provider = Provider }, CancellationToken.None);
